...
function trim_blank_lines(str) {
  let blank_line = new RegExp(/^\s*$/);
  let lines = str.split("\n");
  let result = [];

  let found_non_blank_line = false;

  for (let line of lines) {
    if (found_non_blank_line) {
      result.push(line);
    } else if (!line.match(blank_line)) {
...
export function dedent(str) {
  return vendor_dedent(trim_blank_lines(str.replace(/\t/g, "  ")));
}
